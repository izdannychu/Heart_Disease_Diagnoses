...
export default function ExplorerTab() {
  ...
  const fetchStats = async () => {
    try {
      ...
      console.error(err);
      setError('Không thể kết nối đến máy chủ API hoặc dữ liệu chưa được huấn luyện.');
    } finally {
      setLoading(false);
    }
  };
  ...
  if (error) {
    return (
      <div className="error-container">
        ...
        <button className="btn btn-primary" onClick={fetchStats}>Thử Lại</button>
      </div>
    );
  }
      ...
      <div className="donut-card">
        ...
        <div className="donut-body">
          <svg viewBox="-1.1 -1.1 2.2 2.2" className="donut-svg">
            ...
            <text x="0" y="0" dominantBaseline="middle" textAnchor="middle" fill="#fff" fontSize="0.25" fontWeight="bold">
              {total} mẫu
            </text>
          </svg>
  ...
  const renderCorrelationMatrix = (corr) => {
    ...
    return (
      <div className="heatmap-card">
        ...
        <p className="margin-bottom-sm">Mức độ tương quan tuyến tính giữa các chỉ số số học và nhãn kết quả (HeartDisease).</p>
        <div className="heatmap-scroll">
          <div className="heatmap-grid" style={{ gridTemplateColumns: `repeat(${vars.length + 1}, minmax(80px, 1fr))` }}>
            {/* Header row corner */}
            <div className="heatmap-header-cell"></div>
            {/* Header labels */}
            {vars.map((v, i) => (
              <div key={i} className="heatmap-header-cell font-xs">{v}</div>
            ))}
  ...
  return (
    <div className="tab-pane fade-in">
      <div className="section-header">
        ...
        <p>Thống kê trực quan từ 918 hồ sơ lâm sàng giúp định dạng phân phối dữ liệu huấn luyện.</p>
      </div>
      ...
      <div className="stats-summary-grid">
        ...
        <div className="stat-summary-card bg-glass">
          ...
          <div>
            <h3>{stats.heart_disease_distribution['1']} ({((stats.heart_disease_distribution['1'] / stats.total_records) * 100).toFixed(1)}%)</h3>
        ...
        <div className="stat-summary-card bg-glass">
          ...
          <div>
            <h3>{stats.gender_distribution['M']} / {stats.gender_distribution['F']}</h3>
            <p>Phân bố Nam / Nữ</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid-2-col margin-top-md">
        ...
        <div className="card text-stats-card">
          ...
          <div className="stats-insight-info">
            <AlertCircle size={16} />
            <span>
              Lưu ý: Tập dữ liệu chứa một số bản ghi có chỉ số Cholesterol bằng 0 mg/dl (293 mẫu) do lỗi dữ liệu lúc ghi nhận ở một vài trung tâm y tế ban đầu, mô hình học máy đã được xử lý phù hợp.
            </span>
          </div>
        </div>
      </div>
